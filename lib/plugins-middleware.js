...
module.exports = function(plugins) {
    ...
    if(copyOfPluginsReversed && copyOfPluginsReversed[copyOfPluginsReversed.length-1].id === ANALYTICS && copyOfPluginsReversed[copyOfPluginsReversed.length-2].id === ENVOY){
        let pluginIds           = [];
        let reversedPluginIds   = [];
        let temp = copyOfPluginsReversed[copyOfPluginsReversed.length-1];
        //swap position of envoy with analytics plugin.
        ...
        debug('Plugin sequence',pluginIds);
        copyOfPluginsReversed.map(function(plugin){
            reversedPluginIds.push(plugin.id);
        });
        debug('Reversed plugin sequence',reversedPluginIds);
    }
